@php
    ...
    $subNav = [
        ...
        ['key' => 'kurikulum', 'route' => 'tenant.tenant.kurikulum.index', 'match' => 'tenant.tenant.kurikulum.*', 'label' => 'Kurikulum', 'icon' => 'book'],
        ['key' => 'jurusan', 'route' => 'tenant.tenant.jurusan.index', 'match' => 'tenant.tenant.jurusan.*', 'label' => 'Jurusan', 'icon' => 'tag'],
        ['key' => 'kelas', 'route' => 'tenant.tenant.kelas.index', 'match' => 'tenant.tenant.kelas.*', 'label' => 'Kelas', 'icon' => 'class'],
        ['key' => 'ruangan', 'route' => 'tenant.tenant.ruangan.index', 'match' => 'tenant.tenant.ruangan.*', 'label' => 'Ruangan', 'icon' => 'room'],
        ['key' => 'jabatan', 'route' => 'tenant.tenant.jabatan.index', 'match' => 'tenant.tenant.jabatan.*', 'label' => 'Jabatan', 'icon' => 'badge'],
        ['key' => 'hak-akses', 'route' => 'tenant.hak-akses.index', 'match' => 'tenant.hak-akses.*', 'label' => 'Hak Akses', 'icon' => 'shield'],
    ];
@endphp
                ...
                <li>
                    <a href="{{ route($item['route'], $tenant) }}" class="inline-flex items-center gap-2 whitespace-nowrap rounded-lg px-3 py-2 text-sm font-medium transition {{ $isActive ? 'bg-indigo-50 text-indigo-700 ring-1 ring-indigo-100' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900' }}">
                        ...
                        @elseif ($item['icon'] === 'tag')
                            <svg class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M7 7h.01M7 3h5a1.99 1.99 0 011.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.99 1.99 0 013 12V7a4 4 0 014-4z"/></svg>
