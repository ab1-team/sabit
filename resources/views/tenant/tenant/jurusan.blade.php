...
<body class="min-h-screen text-slate-800">
    ...
    <main class="mx-auto max-w-7xl px-4 py-6 sm:px-6 sm:py-8 lg:px-8">
        ...
        <section class="mt-6 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div class="p-4 sm:p-5 overflow-x-auto">
                <table id="jurusan-table" class="w-full text-sm" style="width:100%">
        ...
        <div class="modal-scroll w-full max-w-2xl max-h-[95vh] overflow-y-auto rounded-2xl bg-white shadow-2xl">
            ...
            <form id="jurusan-form" action="{{ route('tenant.tenant.jurusan.store', $tenant) }}" method="POST" class="px-5 py-5 sm:px-6">
                ...
                <div class="mt-6 flex flex-col gap-2 border-t border-slate-100 pt-4 sm:flex-row sm:justify-end">
                    <button type="submit" id="submit-jurusan" class="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-indigo-600 px-5 text-sm font-semibold text-white hover:bg-indigo-700 focus:outline-none focus:ring-4 focus:ring-indigo-200 sm:w-auto">
                        <svg class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="2.4" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4"/></svg>
    ...
    <script>
        ...
        document.addEventListener('click', function (e) {
            ...
            var delBtn = e.target.closest('.delete-jurusan');
            if (delBtn) {
                Swal.fire({
                    title: 'Hapus jurusan ini?',
                    text: 'Jurusan ' + delBtn.dataset.name + ' akan dihapus. Tindakan ini tidak dapat dibatalkan.',
                    icon: 'warning',
        ...
        $(function () {
            $('#jurusan-table').DataTable({
                ...
                ajax: {
                    url: @json(route('tenant.tenant.jurusan.data', $tenant)),
                    error: function (xhr) {
                        console.error('DataTables jurusan error:', xhr.status, xhr.responseText);
                    }
                },
                columns: [
                    { data: 'DT_RowIndex', name: 'DT_RowIndex', orderable: false, searchable: false, className: 'px-3 py-3 text-slate-500' },
                    { data: 'nama', name: 'nama', className: 'px-3 py-3 font-semibold text-slate-800' },
                    { data: 'kode_jurusan', name: 'kode_jurusan', orderable: true, searchable: true, className: 'px-3 py-3 font-mono text-xs text-slate-700' },
