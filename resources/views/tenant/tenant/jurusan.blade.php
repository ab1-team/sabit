@php
    ...
    $jurusanItem = $jurusanItem ?? new \App\Models\Jurusan();
@endphp
<!DOCTYPE html>
<html lang="id">
<head>
    ...
    <title>Jurusan — {{ env('APP_NAME') }}</title>
...
<body class="min-h-screen text-slate-800">
    ...
    <main class="mx-auto max-w-7xl px-4 py-6 sm:px-6 sm:py-8 lg:px-8">
        <header class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div class="min-w-0">
                ...
                <h2 class="mt-1 text-2xl font-bold tracking-tight text-slate-900 sm:text-3xl">Jurusan</h2>
                <p class="mt-1 text-sm text-slate-500">Jurusan.</p>
            </div>
            <button type="button" id="add-jurusan" class="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-indigo-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm shadow-indigo-600/20 transition hover:bg-indigo-700 focus:outline-none focus:ring-4 focus:ring-indigo-200 sm:w-auto">
        ...
        @include('tenant.partials.tenant_subnav', ['tenant' => $tenant, 'active' => 'jurusan'])
        ...
        <section class="mt-6 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse ($items as $j)
                            <tr class="hover:bg-slate-50">
                                <td class="px-5 py-3 font-semibold text-slate-800">{{ $j->nama }}</td>
                                <td class="px-5 py-3 font-mono text-xs text-slate-700">{{ $j->kode_jurusan }}</td>
                                <td class="px-5 py-3">
                                    @if ($j->status === 'aktif')
                                ...
                                <td class="px-5 py-3 text-center">
                                    <div class="inline-flex items-center gap-1">
                                        <button type="button" class="open-edit-modal inline-flex items-center rounded-lg bg-indigo-100 px-2.5 py-1.5 text-xs font-semibold text-indigo-700 hover:bg-indigo-200"
                                            data-update-url="{{ route('tenant.tenant.jurusan.update', [$tenant, $j]) }}"
                                            data-nama="{{ $j->nama }}"
                                        ...
                                        <button type="button" class="delete-jurusan inline-flex items-center rounded-lg bg-rose-100 px-2.5 py-1.5 text-xs font-semibold text-rose-700 hover:bg-rose-200" data-action="{{ route('tenant.tenant.jurusan.destroy', [$tenant, $j]) }}" data-name="{{ $j->nama }}">
                        ...
                        @empty
                            <tr><td colspan="4" class="px-5 py-14 text-center text-sm text-slate-400">Belum ada jurusan.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
            ...
            <ul class="divide-y divide-slate-100 md:hidden">
                @forelse ($items as $j)
                    <li class="px-4 py-3.5">
                        <div class="flex items-start justify-between gap-2">
                            <div class="min-w-0">
                                <p class="text-sm font-semibold text-slate-900">{{ $j->nama }}</p>
                                <p class="mt-0.5 font-mono text-xs text-slate-600">{{ $j->kode_jurusan }}</p>
                                <div class="mt-1">
                                    @if ($j->status === 'aktif')
                            ...
                            <div class="flex flex-shrink-0 items-center gap-1">
                                ...
                                <button type="button" class="delete-jurusan inline-flex items-center rounded-lg bg-rose-100 p-1.5 text-rose-700 hover:bg-rose-200" data-action="{{ route('tenant.tenant.jurusan.destroy', [$tenant, $j]) }}" data-name="{{ $j->nama }}">
                                    <svg class="h-3.5 w-3.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.48 0 00-7.5 0"/></svg>
                                </button>
                            </div>
                        </div>
                ...
                @empty
                    <li class="px-5 py-14 text-center text-sm text-slate-400">Belum ada jurusan.</li>
                @endforelse
            </ul>
        </section>
    </main>

    <div id="jurusan-modal" class="{{ $initialModalOpen ? 'flex' : 'hidden' }} fixed inset-0 z-50 items-center justify-center bg-slate-900/50 p-2 backdrop-blur-sm sm:p-4" role="dialog" aria-modal="true" aria-labelledby="modal-title">
        <div class="modal-scroll w-full max-w-2xl max-h-[95vh] overflow-y-auto rounded-2xl bg-white shadow-2xl">
            <div class="flex items-start justify-between gap-4 border-b border-slate-100 px-5 py-4 sm:px-6">
                <div>
                    <h3 id="modal-title" class="text-lg font-bold text-slate-900">Tambah Jurusan</h3>
                    <p id="modal-description" class="mt-1 text-sm text-slate-500">Tambahkan data jurusan baru.</p>
                </div>
                <button type="button" id="close-jurusan-modal" aria-label="Tutup dialog" class="rounded-lg p-2 text-slate-400 hover:bg-slate-100 hover:text-slate-700 focus:outline-none focus:ring-4 focus:ring-indigo-100">
            ...
            <form id="jurusan-form" action="{{ route('tenant.tenant.jurusan.store', $tenant) }}" method="POST" class="px-5 py-5 sm:px-6">
                @csrf
                <input type="hidden" name="_method" id="form-method" value="POST">
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    @include('tenant.tenant._formulir_jurusan', ['jurusanItem' => $jurusanItem])
                </div>
                <div class="mt-6 flex flex-col-reverse gap-2 border-t border-slate-100 pt-4 sm:flex-row sm:justify-end">
                    <button type="button" id="cancel-jurusan-modal" class="inline-flex min-h-11 items-center justify-center rounded-xl border border-slate-200 bg-white px-5 text-sm font-semibold text-slate-700 hover:bg-slate-50 focus:outline-none focus:ring-4 focus:ring-slate-100">Batal</button>
                    <button type="submit" id="submit-jurusan" class="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-indigo-600 px-5 text-sm font-semibold text-white hover:bg-indigo-700 focus:outline-none focus:ring-4 focus:ring-indigo-200">
                        <svg class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="2.4" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4"/></svg>
                        <span id="submit-label">Tambah Jurusan</span>
                    </button>
                </div>
            </form>
        </div>
    ...
    <script>
        const jurusanModal = document.getElementById('jurusan-modal');
        const jurusanForm = document.getElementById('jurusan-form');
        const formMethod = document.getElementById('form-method');
        const submitLabel = document.getElementById('submit-label');
        const modalTitle = document.getElementById('modal-title');
        const modalDesc = document.getElementById('modal-description');
        const deleteForm = document.getElementById('delete-jurusan-form');

        function setModalState(open) {
            jurusanModal.classList.toggle('hidden', !open);
            jurusanModal.classList.toggle('flex', open);
            document.body.classList.toggle('overflow-hidden', open);
        }

        function openCreateModal() {
            jurusanForm.action = @json(route('tenant.tenant.jurusan.store', $tenant));
            formMethod.value = 'POST';
            jurusanForm.reset();
            document.getElementById('status').value = 'aktif';
            modalTitle.textContent = 'Tambah Jurusan';
            modalDesc.textContent = 'Tambahkan data jurusan baru.';
            submitLabel.textContent = 'Tambah Jurusan';
            setModalState(true);
            if (window.initFancyInputs) window.initFancyInputs('#jurusan-modal');
            document.getElementById('nama').focus();
        }

        function openEditModal(btn) {
            jurusanForm.action = btn.dataset.updateUrl;
            formMethod.value = 'PUT';
            document.getElementById('nama').value = btn.dataset.nama || '';
            document.getElementById('kode_jurusan').value = btn.dataset.kode || '';
            document.getElementById('status').value = btn.dataset.status || 'aktif';
            modalTitle.textContent = 'Ubah Jurusan';
            modalDesc.textContent = 'Perbarui data jurusan ' + (btn.dataset.nama || '') + '.';
        ...
        document.getElementById('add-jurusan').addEventListener('click', openCreateModal);
        document.querySelectorAll('.open-edit-modal').forEach(function (btn) {
            btn.addEventListener('click', function () { openEditModal(this); });
        });
        document.getElementById('close-jurusan-modal').addEventListener('click', function () { setModalState(false); });
        document.getElementById('cancel-jurusan-modal').addEventListener('click', function () { setModalState(false); });
        jurusanModal.addEventListener('click', function (event) {
            if (event.target === jurusanModal) setModalState(false);
        });
        document.addEventListener('keydown', function (event) {
            if (event.key === 'Escape' && !jurusanModal.classList.contains('hidden')) {
                setModalState(false);
            }
        });

        jurusanForm.addEventListener('submit', function () {
            const btn = document.getElementById('submit-jurusan');
            btn.disabled = true;
            btn.classList.add('opacity-60', 'cursor-not-allowed');
            submitLabel.textContent = 'Menyimpan…';
        });

        document.querySelectorAll('.delete-jurusan').forEach(function (btn) {
            btn.addEventListener('click', function () {
                const action = this.dataset.action;
                const name = this.dataset.name;
                Swal.fire({
                    title: 'Hapus jurusan ini?',
                    text: 'Jurusan ' + name + ' akan dihapus. Tindakan ini tidak dapat dibatalkan.',
                    icon: 'warning',
        ...
        @if ($errors->any())
        (function () {
            var fieldId = @json($errors->keys()[0] ?? '');
            var idMap = { nama: 'nama', kode_jurusan: 'kode_jurusan', status: 'status' };
            var target = idMap[fieldId] || 'nama';
            if (window.initFancyInputs) window.initFancyInputs('#jurusan-modal');
            setTimeout(function () {
