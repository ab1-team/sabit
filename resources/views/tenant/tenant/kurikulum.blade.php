...
<body class="min-h-screen text-slate-800">
    ...
    <main class="mx-auto max-w-7xl px-4 py-6 sm:px-6 sm:py-8 lg:px-8">
        ...
        <section class="mt-6 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div class="hidden md:block overflow-x-auto">
                <table class="min-w-[480px] w-full text-sm">
                    <thead class="bg-slate-50 text-left text-xs uppercase tracking-wider text-slate-500">
                        <tr>
                            <th class="px-5 py-3 font-semibold">Nama Kurikulum</th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse ($items as $k)
                            <tr class="hover:bg-slate-50">
                                <td class="px-5 py-3 font-semibold text-slate-800">{{ $k->nama_kurikulum }}</td>
                                <td class="px-5 py-3 font-mono text-xs text-slate-700">{{ $k->kode_kurikulum ?? '—' }}</td>
                                <td class="px-5 py-3">
                                    @if ($k->status === 'aktif')
                                ...
                                <td class="px-5 py-3 text-center">
                                    <div class="inline-flex items-center gap-1">
                                        <button type="button" class="open-edit-modal inline-flex items-center rounded-lg bg-indigo-100 px-2.5 py-1.5 text-xs font-semibold text-indigo-700 hover:bg-indigo-200"
                                            data-update-url="{{ route('tenant.tenant.kurikulum.update', [$tenant, $k]) }}"
                                            data-nama="{{ $k->nama_kurikulum }}"
                                            data-kode="{{ $k->kode_kurikulum }}"
                                        ...
                                        <button type="button" class="delete-kurikulum inline-flex items-center rounded-lg bg-rose-100 px-2.5 py-1.5 text-xs font-semibold text-rose-700 hover:bg-rose-200" data-action="{{ route('tenant.tenant.kurikulum.destroy', [$tenant, $k]) }}" data-name="{{ $k->nama_kurikulum }}">
                        ...
                        @empty
                            <tr><td colspan="4" class="px-5 py-14 text-center text-sm text-slate-400">Belum ada kurikulum.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
            ...
            <ul class="divide-y divide-slate-100 md:hidden">
                @forelse ($items as $k)
                    <li class="px-4 py-3.5">
                        <div class="flex items-start justify-between gap-2">
                            <div class="min-w-0">
                                <p class="text-sm font-semibold text-slate-900">{{ $k->nama_kurikulum }}</p>
                                <p class="mt-0.5 truncate text-xs text-slate-600">{{ $k->kode_kurikulum ?? '—' }}</p>
                                <div class="mt-1">
                                    @if ($k->status === 'aktif')
                            ...
                            <div class="flex flex-shrink-0 items-center gap-1">
                                ...
                                <button type="button" class="delete-kurikulum inline-flex items-center rounded-lg bg-rose-100 p-1.5 text-rose-700 hover:bg-rose-200" data-action="{{ route('tenant.tenant.kurikulum.destroy', [$tenant, $k]) }}" data-name="{{ $k->nama_kurikulum }}">
                                    <svg class="h-3.5 w-3.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.48 0 00-7.5 0"/></svg>
                                </button>
                            </div>
                        </div>
                ...
                @empty
                    <li class="px-5 py-14 text-center text-sm text-slate-400">Belum ada kurikulum.</li>
                @endforelse
            </ul>
        </section>
    </main>
        ...
        <div class="modal-scroll w-full max-w-2xl max-h-[95vh] overflow-y-auto rounded-2xl bg-white shadow-2xl">
            ...
            <form id="kurikulum-form" action="{{ route('tenant.tenant.kurikulum.store', $tenant) }}" method="POST" class="px-5 py-5 sm:px-6">
                ...
                <div class="mt-6 flex flex-col-reverse gap-2 border-t border-slate-100 pt-4 sm:flex-row sm:justify-end">
                    <button type="button" id="cancel-kurikulum-modal" class="inline-flex min-h-11 items-center justify-center rounded-xl border border-slate-200 bg-white px-5 text-sm font-semibold text-slate-700 hover:bg-slate-50 focus:outline-none focus:ring-4 focus:ring-slate-100">Batal</button>
                    <button type="submit" id="submit-kurikulum" class="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-indigo-600 px-5 text-sm font-semibold text-white hover:bg-indigo-700 focus:outline-none focus:ring-4 focus:ring-indigo-200">
                        <svg class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="2.4" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4"/></svg>
    ...
    <script>
        ...
        document.getElementById('cancel-kurikulum-modal').addEventListener('click', function () { setModalState(false); });
        ...
        document.querySelectorAll('.delete-kurikulum').forEach(function (btn) {
            btn.addEventListener('click', function () {
                const action = this.dataset.action;
                const name = this.dataset.name;
                Swal.fire({
                    title: 'Hapus kurikulum ini?',
                    text: 'Kurikulum ' + name + ' akan dihapus. Tindakan ini tidak dapat dibatalkan.',
                    icon: 'warning',
