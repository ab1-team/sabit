@php
    ...
    $jabatanItem = $jabatanItem ?? new \App\Models\Jabatan();
@endphp
<!DOCTYPE html>
<html lang="id">
<head>
    ...
    <title>Jabatan — {{ env('APP_NAME') }}</title>
...
<body class="min-h-screen text-slate-800">
    ...
    <main class="mx-auto max-w-7xl px-4 py-6 sm:px-6 sm:py-8 lg:px-8">
        <header class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div class="min-w-0">
                ...
                <h2 class="mt-1 text-2xl font-bold tracking-tight text-slate-900 sm:text-3xl">Jabatan</h2>
                <p class="mt-1 text-sm text-slate-500">Referensi jabatan untuk operator tenant {{ $tenant->id }}.</p>
            </div>
            <button type="button" id="add-jabatan" class="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-indigo-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm shadow-indigo-600/20 transition hover:bg-indigo-700 focus:outline-none focus:ring-4 focus:ring-indigo-200 sm:w-auto">
    ...
    <div id="jabatan-modal" class="{{ $initialModalOpen ? 'flex' : 'hidden' }} fixed inset-0 z-50 items-center justify-center bg-slate-900/50 p-2 backdrop-blur-sm sm:p-4" role="dialog" aria-modal="true" aria-labelledby="modal-title">
        <div class="modal-scroll w-full max-w-2xl max-h-[95vh] overflow-y-auto rounded-2xl bg-white shadow-2xl">
            <div class="flex items-start justify-between gap-4 border-b border-slate-100 px-5 py-4 sm:px-6">
                <div>
                    <h3 id="modal-title" class="text-lg font-bold text-slate-900">Tambah Jabatan</h3>
                    <p id="modal-description" class="mt-1 text-sm text-slate-500">Tambahkan referensi jabatan baru.</p>
                </div>
                <button type="button" id="close-jabatan-modal" aria-label="Tutup dialog" class="rounded-lg p-2 text-slate-400 hover:bg-slate-100 hover:text-slate-700 focus:outline-none focus:ring-4 focus:ring-indigo-100">
            ...
            <form id="jabatan-form" action="{{ route('tenant.tenant.jabatan.store', $tenant) }}" method="POST" class="px-5 py-5 sm:px-6">
                @csrf
    ...
    <form id="delete-jabatan-form" method="POST" class="hidden">
        @csrf
        @method('DELETE')
    </form>
    ...
    <script>
        ...
        const deleteForm = document.getElementById('delete-jabatan-form');

        function setModalState(open) {
            jabatanModal.classList.toggle('hidden', !open);
            jabatanModal.classList.toggle('flex', open);
            document.body.classList.toggle('overflow-hidden', open);
        }

        function openCreateModal() {
            jabatanForm.action = @json(route('tenant.tenant.jabatan.store', $tenant));
            formMethod.value = 'POST';
            jabatanForm.reset();
            modalTitle.textContent = 'Tambah Jabatan';
            modalDesc.textContent = 'Tambahkan referensi jabatan baru.';
            submitLabel.textContent = 'Tambah Jabatan';
            setModalState(true);
            if (window.initFancyInputs) window.initFancyInputs('#jabatan-modal');
            document.getElementById('nama_jabatan').focus();
        }

        function openEditModal(btn) {
            jabatanForm.action = btn.dataset.updateUrl;
            formMethod.value = 'PUT';
            document.getElementById('nama_jabatan').value = btn.dataset.nama || '';
            document.getElementById('kode_jabatan').value = btn.dataset.kode || '';
            modalTitle.textContent = 'Ubah Jabatan';
            modalDesc.textContent = 'Perbarui referensi jabatan ' + (btn.dataset.nama || '') + '.';
        ...
        document.addEventListener('keydown', function (event) {
            if (event.key === 'Escape' && !jabatanModal.classList.contains('hidden')) {
                setModalState(false);
            }
        });

        jabatanForm.addEventListener('submit', function () {
            const btn = document.getElementById('submit-jabatan');
            btn.disabled = true;
