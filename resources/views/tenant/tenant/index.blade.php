...
<body class="min-h-screen text-slate-800">
    ...
    <main class="mx-auto max-w-7xl px-4 py-6 sm:px-6 sm:py-8 lg:px-8">
        <header class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div class="min-w-0">
                ...
                <p class="mt-1 text-sm text-slate-500">Setiap tenant punya database dan subdomain sendiri.</p>
            </div>
                ...
                <table class="min-w-[640px] w-full text-sm">
                    <thead class="bg-slate-50 text-left text-xs uppercase tracking-wider text-slate-500">
                        <tr>
                            ...
                            <th class="px-5 py-3 font-semibold">Domain</th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                            ...
                            <tr class="hover:bg-slate-50">
                                ...
                                <td class="px-5 py-3">
                                    @foreach ($t->domains as $d)
                                        <span class="inline-flex rounded-md bg-indigo-50 px-2 py-1 text-xs font-medium text-indigo-700">{{ $d->domain }}</span>
                                    @endforeach
                                </td>
                                <td class="px-5 py-3 text-right">
                                    <div class="inline-flex items-center gap-1">
                                        <button type="button" class="open-detail-modal inline-flex items-center rounded-lg bg-slate-100 px-2.5 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-200" title="Detail"
                                            ...
                                            data-domain="{{ optional($t->domains->first())->domain ?? '—' }}"
                                            data-email="{{ $t->email ?? '—' }}"
                                            data-db="tenant{{ $t->id }}"
                                        ...
                                        <button type="button" class="open-edit-modal inline-flex items-center rounded-lg bg-amber-100 px-2.5 py-1.5 text-xs font-semibold text-amber-700 hover:bg-amber-200" title="Edit"
                        ...
                        @empty
                            <tr><td colspan="4" class="px-5 py-14 text-center text-sm text-slate-400">Belum ada tenant.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
                    ...
                    <li class="px-4 py-4">
                        <div class="flex items-start justify-between gap-2">
                            <div class="min-w-0">
                                ...
                                @foreach ($t->domains as $d)
                                    <span class="mt-1 inline-flex rounded-md bg-indigo-50 px-2 py-0.5 text-[11px] font-medium text-indigo-700">{{ $d->domain }}</span>
                                @endforeach
                            </div>
                            <div class="flex flex-shrink-0 items-center gap-1">
                                ...
                                <button type="button" class="open-edit-modal inline-flex items-center rounded-md bg-amber-100 p-1.5 text-amber-700 hover:bg-amber-200" title="Edit"
                                    data-edit-url="{{ route('tenant.tenant.update', $t) }}"
                                    data-id="{{ $t->id }}"
                                    data-nama="{{ $t->nama_sekolah ?? '' }}"
                                    data-domain="{{ optional($t->domains->first())->domain ?? '' }}"
        ...
        <div class="modal-scroll w-full max-w-lg rounded-2xl bg-white shadow-2xl">
            <div class="flex items-start justify-between gap-4 border-b border-slate-100 px-5 py-4 sm:px-6">
                <div>
                    ...
                    <p id="modal-description" class="mt-1 text-sm text-slate-500">Database <span class="font-mono">tenant&lt;id&gt;</span> akan dibuat otomatis.</p>
                </div>
                <button type="button" id="close-tenant-modal" aria-label="Close dialog" class="rounded-lg p-2 text-slate-400 hover:bg-slate-100 hover:text-slate-700 focus:outline-none focus:ring-4 focus:ring-indigo-100">
            ...
            <form id="tenant-form" method="POST" class="px-5 py-5 sm:px-6">
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    @include('tenant.tenant._form', ['tenant' => new \App\Models\Tenant()])
                </div>
    ...
    <div id="detail-modal" class="hidden fixed inset-0 z-50 items-center justify-center bg-slate-900/50 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="detail-modal-title">
        <div class="modal-scroll w-full max-w-lg rounded-2xl bg-white shadow-2xl">
            <div class="flex items-start justify-between gap-4 border-b border-slate-100 px-5 py-4 sm:px-6">
                ...
                <button type="button" id="close-detail-modal" aria-label="Close dialog" class="rounded-lg p-2 text-slate-400 hover:bg-slate-100 hover:text-slate-700 focus:outline-none focus:ring-4 focus:ring-indigo-100">
                    <svg class="h-5 w-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg>
                </button>
            </div>
            <div class="px-5 py-5 sm:px-6">
                <dl class="divide-y divide-slate-100 rounded-xl border border-slate-200">
                    ...
                    <div class="flex items-center justify-between px-4 py-3">
                        ...
                        <dd id="detail-domain" class="font-mono text-sm text-slate-800"></dd>
                    </div>
                    <div class="flex items-center justify-between px-4 py-3">
    ...
    <script>
        ...
        function openDetailModal(btn) {
            ...
            document.getElementById('detail-domain').textContent = btn.dataset.domain;
        ...
        function openCreateModal() {
            ...
            modalDescription.innerHTML = 'Database <span class="font-mono">tenant&lt;id&gt;</span> akan dibuat otomatis.';
            ...
            setModalState(true);
            document.getElementById('tenant-nama').focus();
        }
        ...
        function openEditModal(btn) {
            ...
            modalDescription.textContent = 'Perbarui nama, domain, dan email admin.';
            submitLabel.textContent = 'Simpan Perubahan';
            document.getElementById('tenant-nama').value = btn.dataset.nama || '';
            document.getElementById('tenant-id').value = btn.dataset.id || '';
            document.getElementById('tenant-domain').value = btn.dataset.domain || '';
            document.getElementById('tenant-email').value = btn.dataset.email || '';
