...
<div class="px-2 py-2">
    ...
    @else
        <div class="lp-ps-stack">
            @foreach ($items as $row)
                ...
                <form action="{{ route('app.landing.profile-sections.update', $row->id) }}"
    ...
    <form action="{{ route('app.landing.struktur.store') }}" method="POST"
    ...
    @else
        @foreach ($strukturItems as $row)
            ...
            @endphp
            <form action="{{ route('app.landing.struktur.update', $row->id) }}" method="POST"
                ...
                <div class="lp-ps-list-foot">
                    ...
                    <div style="display:flex;gap:.35rem;">
                        ...
                        <button type="button" class="btn btn-outline-danger" data-lp-delete
                                data-url="{{ route('app.landing.struktur.destroy', $row->id) }}"
    ...
    {{-- Form tambah baru (inline) --}}
    <form action="{{ route('app.landing.fasilitas.store') }}" method="POST"
    ...
    @else
        @foreach ($fasilitasItems as $row)
            <form action="{{ route('app.landing.fasilitas.update', $row->id) }}" method="POST"
                  class="lp-ajax lp-ps-list-card {{ $row->is_published ? '' : 'is-inactive' }}">
                @csrf
                @method('PUT')
                <div class="lp-ps-list-head">
                ...
                <div class="lp-ps-list-foot">
                    ...
                    <div style="display:flex;gap:.35rem;">
                        ...
                        <button type="button" class="btn btn-outline-danger" data-lp-delete
                                data-url="{{ route('app.landing.fasilitas.destroy', $row->id) }}"
